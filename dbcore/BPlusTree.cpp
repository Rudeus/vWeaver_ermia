...
BPlusTreeNode* New_BPlusTreeNode() {
  size_t size_node = sizeof(struct BPlusTreeNode);
	struct BPlusTreeNode* p = (struct BPlusTreeNode*)MM::allocate(size_node);
	p->isRoot = false;
	p->isLeaf = false;
	p->key_num = 0;
	p->child[0] = NULL;
...
BPlusTreeRoot* New_BPlusTreeRoot() {
  size_t size_root = sizeof(struct BPlusTreeRoot);
	struct BPlusTreeRoot* r = (struct BPlusTreeRoot*)MM::allocate(size_root);
	size_t size_node = sizeof(struct BPlusTreeNode);
	struct BPlusTreeNode* p = (struct BPlusTreeNode*)MM::allocate(size_node);
	p->isRoot = false;
	p->isLeaf = false;
